...
function Upload({ token }) {
  ...
  // BULLETPROOF MATH: Checks for both uppercase and lowercase
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #ddd' }}>
            ...
            <tbody>
              {transactions.map((t, idx) => {
                ...
                const rawAmt = t.amount !== undefined ? t.amount : t.Amount;
                const amt = parseFloat(rawAmt);
                const safeDate = t.date || t.Date;
                const safeDesc = t.description || t.Description;

                return (
                  <tr key={idx} style={{ borderBottom: '1px solid #eee' }}>
                    <td style={{ padding: '12px' }}>{safeDate}</td>
                    <td style={{ padding: '12px' }}>{safeDesc}</td>
